...
def get_data(ticker):
    stock_data = yf.download(ticker, start='2024-05-05')
...
def get_differncing_order(close_price):
...
def fit_model(data, differncing_order):
    model = ARIMA(data, order=(30, differncing_order, 30))
    model_fit = model.fit()
    forecast_steps = 30
...
def evaluate_model(original_price, differncing_order):
    train_data, test_data = original_price[:-30], original_price[-30:]
    predictions = fit_model(train_data, differncing_order)
...
def get_forecast(original_price, differncing_order):
    predictions = fit_model(original_price, differncing_order)
